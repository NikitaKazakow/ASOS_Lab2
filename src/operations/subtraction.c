...
bool send_double_to_queue(double value) {
    ...
    if(mq_send(queueDescriptor, str, 6, 30) != -1) {
        printf("Основной процесс: %s %lf %s\n", "Сообщение:", value, "отправлено");
        mq_close(queueDescriptor);
        return true;
    }
    else {
...
int main(int argc, char *argv []) {
    ...
    while(true) {
        ...
        printf("Процесс вычитания: значение %lf прочитано из очереди\n", a);
        ...
        {
            printf("Процесс вычитания: Ошибка отправки значения: %lf\n", result);
            return -1;
        }
        else
        {
            printf("Процесс вычитания: Результат: %lf - %lf = %lf\n", a, b, result);
